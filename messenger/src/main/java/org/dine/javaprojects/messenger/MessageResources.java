...
//			for(long num: toList) {
//				Sim sim=ses.get(Sim.class, num);
//				if(sim==null) {
//				 sim=new Sim();
//				 sim.setSimNumber(num);
//				}
//				
//				sim.getMsgList().add(msg);
//				ses.saveOrUpdate(sim);
//				
//
...
//		SessionFactory sf = configuration.buildSessionFactory(serviceRegistry);
		...
		System.out.println("list extracted. getMsgList()="+(resultList.get(0)).getMsgList());
		if(resultList.size()==0) {
			return null;
		}
		System.out.println("resultlist created");
		List<Message> inList=null;
		try {
			 inList=resultList.get(0).getMsgList();
			System.out.println("inlist created"+inList);
		}catch(Exception ex) {
